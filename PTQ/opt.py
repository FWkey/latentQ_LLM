...
@torch.no_grad()
def opt_sequential(model, dataloader, dev, local_args=None):
    ...
    for i in range(len(layers)):
        ...
        if hasattr(args,'smoothquant') and args.smoothquant:
            ...
            if args.act_fun == 'static':
                ...
                handles.append(layer.fc1.register_forward_hook(sta_batch1))
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                quantizers['model.decoder.layers.%d.%s' % (i, name)] = (gptq[name].quantizer.cpu(), scale.cpu(), zero.cpu(), g_idx.cpu(), args.wbits, args.groupsize)
...
@torch.no_grad()
def opt_eval(model, testenc, dev, local_args=None):
    if local_args is None:
        global args
    else:
        args = local_args
