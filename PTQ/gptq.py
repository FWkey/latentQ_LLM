...
class GPTQ:
    ...
    def naivequant(self, blocksize=128, percdamp=.01, groupsize=-1, actorder=False, name=''):
        self.method = 'naive'
    ...
    def searchquant(self, blocksize=128, percdamp=.01, groupsize=-1, actorder=False, name=''):
        self.method = 'search'
        W = self.layer.weight.data.clone()
        org_shape = W.shape
        ...
        # W_max = W.view(-1, groupsize).amax(dim=-1, keepdim=True)
        # W_min = W.view(-1, groupsize).amin(dim=-1, keepdim=True)
        grid = 20
        # best = torch.full([W.shape[0]], float('inf'), device=device, dtype=dtype)
        best_scale = torch.ones([W.shape[1] // groupsize, W.shape[0]],dtype=torch.float16, device=device)
        best_zero = torch.ones([W.shape[1] // groupsize, W.shape[0]],dtype=torch.float16, device=device)
        # org_out = self.layer(self.inp1)
        ...
        self.inp1 = None
        return best_scale, best_zero, g_idx, None
